...
class Cards {
  ...
  List<Color> arcanaSecondaryColors = [
    Color(0xFFCBDC75),
    Color(0xffdd7b61),
    Color(0xffdda061),
    Color(0xff507b90),
  ...
  List<Color> arcanaTertiaryColors = [
    Color(0xffedf982),
    Color(0xffffc0af),
    Color(0xffffd7af),
    Color(0xff8cafc0),
  ...
  List<String> futurePositionArcanaDescriptions = [
    ...
    "Oschon s’égare, décidant de s’éloigner des pas guidés par Llymlaen. Le Vagabond refuse que son esprit libre soit guidé par quelqu’un d’autre et retrouve l’errance qui le caractérise tant. Llymlaen est secrètement jalouse de l’amour que porte Oschon pour Menphina alors qu’elle est toujours à ses côtés. La Flèche renversée peut signifier une situation compliquée, une errance dans la vie, qui peut parfois mener à une impasse ou pire encore. L’avancée peut être inéluctable et aucune marche arrière n’est possible. L'errance vous guette et vous ne savez pas encore déterminer si c'est une bonne ou une mauvaise chose. Vos objectifs perdus de vue, vous avancerez par hasard au gré des expériences qui seront pour certaines fort désagréables. Borné, vous refusez que l'on vous dicte votre conduite mais vous devez néanmoins prendre le temps de réfléchir, de vous arrêter en plein chemin, reconsidérer la situation et réorienter vos objectifs. Cela peut également être le signe d’une rencontre ou d’une connaissance qui cherchera à vous manipuler, quelqu’un de potentiellement jaloux, qui vous veut pour elle ou vous utiliser. Il est également possible qu’il s’agisse d’une personne dans le besoin, ayant besoin de votre guidance et vos conseils. </div>",
    "La fureur impitoyable de la Conquérente et l’amour de Menphina ont du mal à coexister, créant un écart important entre elles. Un conflit important et violent vous attend. Il se peut que vous perdriez beaucoup de cette rencontre et un choix vous sera imposé. Peu importe ce que vous déciderez, vous risquez de perdre quelque chose dans les deux situations. Néanmoins, vous pouvez encore limiter les pertes, reconsidérer vos relations et vous armer de volonté face à l'adversité. L'issue de ce conflit dépendra de votre résolution, n'attendez pas que l'issue soit décidée par l'autre parti, prenez les devants. C’est aussi l’annonce d’une fin ou d’une épreuve très importante dans une relation, de dissensions ou d’attentes déçues. </div>",
  ];

  AssetImage cardBack = AssetImage('assets/cards/Back.png');
  List<String> positionText = [
    "Passé",
    "Présent",
    "Futur",
  ];
  ...
  Container getArcanaDescription(List<List<int>> drawnCards, num activePage) {
    String arcanaSymbols;
    String arcanaDescription;
    Color textColor = Colors.black;
    if (drawnCards[activePage][0] != -7) {
      switch (activePage) {
        case 0:
          arcanaSymbols = arcanaAllSymbols[drawnCards[activePage][0]];
          arcanaDescription =
              pastPositionArcanaDescriptions[drawnCards[activePage][0]];
          break;
        case 1:
          arcanaSymbols = arcanaAllSymbols[drawnCards[activePage][0]];
          arcanaDescription =
              presentPositionArcanaDescriptions[drawnCards[activePage][0]];
          break;
        case 2:
          arcanaSymbols = arcanaAllSymbols[drawnCards[activePage][0]];
          arcanaDescription =
              futurePositionArcanaDescriptions[drawnCards[activePage][0]];
          break;
      }
    } else
      arcanaDescription = ' ';
...
// TODO: Add color by arcana
  Widget displayArcanaName(List<List<int>> drawnCards, int lastCardDrawn,
    ...
    if (drawnCards[activePage][0] != -7) {
      arcanaNameText = arcanaNames[drawnCards[activePage][0]];
    } else if (!(drawnCards[activePage][0] == -7) && lastCardDrawn >= 0)
      arcanaNameText = arcanaNames[lastCardDrawn];
    else
      arcanaNameText = ' ';
    if (drawnCards[activePage][0] != -7)
      textColor = arcanaMainColors[drawnCards[activePage][0]];
  ...
  Widget displayArcanaSymbols(List<List<int>> drawnCards, int lastCardDrawn,
    ...
    if (drawnCards[activePage][0] != -7) {
      arcanaSymbolsText =
          "« " + arcanaSymbols[drawnCards[activePage][0]] + " »";
    } else if (!(drawnCards[activePage][0] == -7) && lastCardDrawn >= 0)
      arcanaSymbolsText = "« " + arcanaSymbols[lastCardDrawn] + " »";
    else
      arcanaSymbolsText = ' ';
    if (drawnCards[activePage][0] != -7)
      textColor = arcanaMainColors[drawnCards[activePage][0]];
